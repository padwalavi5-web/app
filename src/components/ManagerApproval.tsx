        ...
        {reports.length === 0 ? (
          ...
          reports.map((report) => (
            <div key={report.id} className="border p-4 rounded-xl shadow-sm bg-white">
              ...
              <div className="font-bold text-blue-600 mb-3">{report.totalHours.toFixed(1)} שעות</div>
              {report.details && <div className="mb-3 text-sm bg-slate-50 p-2 rounded-lg">{report.details}</div>}
      ...
      {selectedReport && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded-2xl w-full max-w-sm text-right">
            ...
            />
            <div className="flex gap-2">
              <button onClick={handleReject} className="flex-1 bg-red-600 text-white p-2 rounded-lg">
                שלח דחייה
              </button>
              <button onClick={() => setSelectedReport(null)} className="flex-1 bg-gray-200 p-2 rounded-lg">
                ביטול
              </button>
            </div>
          </div>
