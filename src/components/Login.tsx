    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4" dir="rtl">
      <div className="bg-white rounded-3xl shadow-xl p-8 w-full max-w-md border border-gray-100">
          ...
          {(['youth', 'manager', 'guide'] as Role[]).map((r) => {
            ...
            return (
              <button
                key={r}
                onClick={() => setRole(r)}
                className={`p-3 rounded-xl border-2 transition-all ${role === r ? 'border-blue-500 bg-blue-50' : 'border-gray-100 opacity-60'}`}
              >
        ...
        <div className="space-y-4">
          ...
          <button
            onClick={handleLogin}
            className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold text-lg shadow-lg hover:bg-blue-700 mt-4"
          >
            התחברות
          </button>
        </div>
